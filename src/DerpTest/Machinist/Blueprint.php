...
class Blueprint
{
    ...
    public function make($overrides = array())
    {
        ...
        $machine = new \DerpTest\Machinist\Machine($store, $table, (array)$new_row);

        $related = array_filter($data, function ($i) {
            return is_object($i);
        });
    ...
    public function find($data)
    {
        ...
        if (is_array($this->defaults)) {
            $relationships = array_filter($this->defaults, function ($i) {
                return $i instanceof \DerpTest\Machinist\Relationship;
            });
        } else {
            $relationships = false;
        }
        foreach ($rows as $row) {
            $machine = new \DerpTest\Machinist\Machine($store, $table, (array)$row);
